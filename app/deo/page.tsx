...
export default function DEOPortalPage() {
  ...
  return (
    <div className="min-h-screen pb-16 font-sans">
      ...
      <header className="glass-card mb-6 p-4 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-start">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-gradient-to-tr from-amber-500 to-emerald-500 shadow-lg shadow-amber-500/20 text-white font-bold flex-shrink-0">
              <ClipboardList className="w-6 h-6 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-bold text-white tracking-tight">DEO Data Entry Desk</h1>
                <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-md badge-amber">
                  Operator Desk
                </span>
              </div>
              <p className="text-xs text-gray-400">AICTS Pune · Cardiac Registry Data Capture & Completeness Workstation</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          <button
            onClick={loadData}
            disabled={loading}
            className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl btn-ghost text-xs font-medium"
          >
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl btn-outline text-xs font-semibold"
          >
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="kpi-card p-4 rounded-xl flex flex-col justify-between">
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">Active patients in registry</p>
          </div>

          <div className="kpi-card p-4 rounded-xl flex flex-col justify-between">
            <div className="flex items-center justify-between text-gray-400 mb-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-emerald-500">Complete (Grade A)</span>
              <CheckCircle2 className="w-4 h-4 text-emerald-500" />
            </div>
            <div className="text-2xl font-bold text-emerald-400 tracking-tight">{stats.gradeA}</div>
            <p className="text-[11px] text-gray-400 mt-0.5">&ge;80% complete audit</p>
          </div>

          <div className="kpi-card p-4 rounded-xl flex flex-col justify-between">
            <div className="flex items-center justify-between text-gray-400 mb-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-blue-400">Adequate (Grade B)</span>
              <Activity className="w-4 h-4 text-blue-400" />
            </div>
            <div className="text-2xl font-bold text-blue-400 tracking-tight">{stats.gradeB}</div>
            <p className="text-[11px] text-gray-400 mt-0.5">60–79% complete audit</p>
          </div>

          <div className="kpi-card p-4 rounded-xl flex flex-col justify-between">
            <div className="flex items-center justify-between text-gray-400 mb-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-amber-500">Pending (Grade C/D)</span>
              <AlertTriangle className="w-4 h-4 text-amber-500" />
            </div>
            <div className="text-2xl font-bold text-amber-400 tracking-tight">{stats.needAction}</div>
            <p className="text-[11px] text-gray-400 mt-0.5">Missing labs or vitals</p>
          </div>

          <div className="col-span-2 lg:col-span-1 kpi-card p-4 rounded-xl flex flex-col justify-between">
            <div className="flex items-center justify-between text-gray-400 mb-1">
              <span className="text-xs font-semibold uppercase tracking-wider text-blue-400">Avg Completeness</span>
              <Sparkles className="w-4 h-4 text-amber-400" />
            </div>
            <div className="text-2xl font-bold text-white tracking-tight">{stats.avgPct}%</div>
            <div className="w-full bg-gray-700/30 h-1.5 rounded-full mt-1.5 overflow-hidden">
              <div className="bg-gradient-to-r from-blue-500 to-emerald-500 h-full rounded-full transition-all duration-500" style={{ width: `${stats.avgPct}%` }} />
            </div>
          </div>
        </div>

        {/* ── Main Tab Switcher ── */}
        <div className="flex flex-wrap items-center gap-2 border-b border-gray-700/20 pb-3">
          <button
          ...
          >
            ...
            {stats.needAction > 0 && (
              <span className={`px-1.5 py-0.2 rounded-full text-[10px] font-bold ${
                activeTab === 'worklist' ? 'bg-slate-950 text-amber-400' : 'badge-amber'
              }`}>
                {stats.needAction}
              </span>
            )}
        ...
        {activeTab === 'worklist' && (
          <div className="space-y-4">
            <div className="glass-card p-4 rounded-xl flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="search-input w-full pl-9 pr-4 py-2 text-xs"
                />
              </div>

              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-400 font-medium">Quality Grade:</span>
                {(['ALL', 'D', 'C', 'B', 'A'] as const).map(grade => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${
                      filterGrade === grade
                        ? grade === 'D' || grade === 'C' ? 'bg-amber-500 text-slate-950 font-bold' : 'btn-primary'
                        : 'btn-ghost'
                    }`}
                  >
            ...
            <div className="glass-card rounded-xl overflow-hidden shadow-xl">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse registry-table">
                  <thead>
                    <tr className="border-b border-gray-700/20 text-[11px] font-semibold uppercase tracking-wider">
                    ...
                    {loading ? (
                      ...
                      <tr>
                        <td colSpan={5} className="py-12 text-center text-gray-400">
                          ...
                          <p className="text-xs text-gray-400 mt-1">All selected records meet data completeness standards.</p>
                        </td>
                      </tr>
                    ) : (
                      worklistRows.map(row => {
                        ...
                        return (
                          <tr key={row.patient.id} className="transition-colors">
                            ...
                            <td className="py-3.5 px-4">
                              ...
                              <div className="text-[11px] text-gray-400 truncate max-w-[140px]">{row.patient.indexEtiology?.[0] || 'Heart Failure'}</div>
                            </td>

                            <td className="py-3.5 px-4">
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded font-bold text-[11px] ${
                                  row.dataGrade === 'A' ? 'badge-green' :
                                  row.dataGrade === 'B' ? 'badge-blue' :
                                  row.dataGrade === 'C' ? 'badge-amber' :
                                  'badge-red'
                                }`}>
                                  Grade {row.dataGrade} ({row.completenessPct}%)
                                </span>
                              </div>
                              <div className="w-28 bg-gray-700/30 h-1.5 rounded-full mt-1.5 overflow-hidden">
                                <div
                                  className={`h-full rounded-full ${
                                    row.completenessPct >= 80 ? 'bg-emerald-500' :
                                    row.completenessPct >= 60 ? 'bg-blue-500' :
                                    row.completenessPct >= 40 ? 'bg-amber-500' : 'bg-red-500'
                                  }`}
                              ...
                              {row.missingLabels.length === 0 ? (
                                ...
                                <div className="flex flex-wrap gap-1 max-w-md">
                                  {row.missingLabels.slice(0, 4).map((label, idx) => (
                                    <span key={idx} className="badge-red px-2 py-0.5 rounded text-[10px] font-medium">
                                      {label}
                                    </span>
                                  ))}
                                  {row.missingLabels.length > 4 && (
                                    <span className="badge-gray px-1.5 py-0.5 rounded text-[10px] font-medium">
        ...
        ───────────────────────────────────────────────────────────── */}
        {activeTab === 'register' && (
          <div className="max-w-4xl mx-auto glass-card p-6 rounded-2xl shadow-xl space-y-6">
            <div className="border-b border-gray-700/20 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Capture essential demographics and clinical etiology for new registry admission</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleRegisterPatient} className="space-y-6">
              {/* Demographics Grid */}
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-amber-500 mb-3">1. Patient Demographics & Hospital Identification</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">First Name *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Last Name *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Hospital ID / MRN (Auto if blank)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Age (Years)</label>
                    <input
                      type="number"
                      placeholder="e.g. 58"
                      value={regForm.age}
                  ...
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Biological Sex</label>
                    <select
                      value={regForm.sex}
                      onChange={e => setRegForm({ ...regForm, sex: e.target.value as any })}
                      className="form-select w-full px-3 py-2 text-xs"
                    >
                  ...
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Contact Phone</label>
                    <input
                      type="tel"
              ...
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-amber-500 mb-3">2. Baseline Clinical Classification</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Primary Etiology</label>
                    <select
                  ...
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Baseline NYHA Class</label>
                    <select
                      value={regForm.nyha}
                  ...
                  <div>
                    <label className="form-label block text-[11px] font-medium mb-1">Baseline LVEF (%)</label>
                    <input
                      type="number"
                      placeholder="e.g. 30"
                      value={regForm.lvef}
              ...
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-amber-500 mb-2">3. Documented Comorbidities</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                  {[
                    ...
                    <label key={c.key} className="dark-card flex items-center gap-2 p-2.5 rounded-lg cursor-pointer">
                      <input
                        type="checkbox"
                        checked={(regForm as any)[c.key]}
                        onChange={e => setRegForm({ ...regForm, [c.key]: e.target.checked })}
                        className="rounded border-gray-400 text-blue-600 focus:ring-0"
                      />
              ...
              {/* Submit Buttons */}
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-700/20">
                <button
                  type="button"
                  onClick={() => setActiveTab('worklist')}
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold shadow-lg shadow-emerald-500/20 transition-all"
                >
        ...
        {activeTab === 'encounter' && (
          <div className="max-w-4xl mx-auto glass-card p-6 rounded-2xl shadow-xl space-y-6">
            <div className="border-b border-gray-700/20 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Log vitals, 2D Echo measurements, 12-lead ECG, blood labs, and 4-pillar GDMT</p>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleLogEncounter} className="space-y-6">
              {/* Select Patient & Date */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl dark-card">
                <div>
                  <label className="form-label block text-[11px] font-semibold text-amber-500 mb-1">Select Patient *</label>
                  <select
                    required
                    value={encounterPatientId}
                    onChange={e => setEncounterPatientId(e.target.value)}
                    className="form-select w-full px-3 py-2 text-xs font-medium"
                  >
                ...
                <div>
                  <label className="form-label block text-[11px] font-semibold mb-1">Encounter Date *</label>
                  <input
                    type="date"
                    required
              ...
              {/* Vitals Grid */}
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-blue-500 mb-3">1. Vital Signs & Physical Findings</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                  <div>
                    <label className="form-label block text-[10px] mb-1">Systolic BP (mmHg)</label>
                    <input
                      type="number"
                      placeholder="120"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Diastolic BP (mmHg)</label>
                    <input
                      type="number"
                      placeholder="80"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Heart Rate (bpm)</label>
                    <input
                      type="number"
                      placeholder="72"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">SpO2 (%)</label>
                    <input
                      type="number"
                      placeholder="98"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Weight (kg)</label>
                    <input
                      type="number"
                      placeholder="65"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Oedema Grade</label>
                    <select
                      value={encounterForm.oedema || 'None'}
                      onChange={e => setEncounterForm({ ...encounterForm, oedema: e.target.value as any })}
                      className="form-select w-full px-2.5 py-1.5 text-xs"
                    >
              ...
              {/* 2D Echo & ECG Grid */}
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-blue-500 mb-3">2. 2D Echocardiography & 12-Lead ECG</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
                  <div>
                    <label className="form-label block text-[10px] mb-1">LVEF (%)</label>
                    <input
                      type="number"
                      placeholder="30"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">LVEDD (mm)</label>
                    <input
                      type="number"
                      placeholder="55"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">TAPSE (mm)</label>
                    <input
                      type="number"
                      placeholder="18"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">E/e' Ratio</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="14"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">RVSP (mmHg)</label>
                    <input
                      type="number"
                      placeholder="35"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">QRS (ms)</label>
                    <input
                      type="number"
                      placeholder="110"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Conduction/BBB</label>
                    <select
              ...
              {/* Lab Parameters Grid */}
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-blue-500 mb-3">3. Blood Laboratory Parameters</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3">
                  <div>
                    <label className="form-label block text-[10px] mb-1">Creatinine</label>
                    <input
                      type="number"
                      step="0.01"
                      placeholder="1.0"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">eGFR</label>
                    <input
                      type="number"
                      placeholder="75"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Potassium</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="4.2"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Sodium</label>
                    <input
                      type="number"
                      placeholder="138"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Hb (g/dL)</label>
                    <input
                      type="number"
                      step="0.1"
                      placeholder="12.5"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">NT-proBNP</label>
                    <input
                      type="number"
                      placeholder="1200"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Ferritin</label>
                    <input
                      type="number"
                      placeholder="150"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">TSAT (%)</label>
                    <input
                      type="number"
                      placeholder="25"
              ...
              {/* 4-Pillar GDMT Check */}
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-blue-500 mb-3">4. 4-Pillar Guideline-Directed Medical Therapy (GDMT)</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={pillar.key} className="p-3 rounded-xl dark-card space-y-2">
                      <div className="text-[11px] font-bold">{pillar.title}</div>
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          className="form-select w-full px-2 py-1 text-xs"
                        >
        ...
        {activeTab === 'directory' && (
          <div className="space-y-4">
            <div className="glass-card p-4 rounded-xl flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <input
                  type="text"
              ...
              <button
                onClick={() => setActiveTab('register')}
                className="btn-primary flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all"
              >
                <UserPlus className="w-4 h-4" />
                Add New Patient
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {processedPatients
                ...
                .map(row => {
                  ...
                  return (
                    <div key={row.patient.id} className="glass-card p-4 rounded-xl flex flex-col justify-between space-y-3">
                      <div>
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            row.dataGrade === 'A' ? 'badge-green' :
                            row.dataGrade === 'B' ? 'badge-blue' :
                            row.dataGrade === 'C' ? 'badge-amber' : 'badge-red'
                          }`}>
                            Grade {row.dataGrade} ({row.completenessPct}%)
                          </span>
                        </div>
      ...
      {isEditModalOpen && selectedPatientForEdit && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in overflow-y-auto">
          <div className="glass-card relative w-full max-w-3xl my-8 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSaveQuickEdit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
              ...
              {/* Vitals Section */}
              <div>
                <h4 className="text-[11px] font-bold uppercase tracking-wider text-amber-500 mb-2">Vital Signs & Physical State</h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                  <div>
                    <label className="form-label block text-[10px] mb-1">Systolic BP</label>
                    <input
                      type="number"
                      placeholder="mmHg"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Diastolic BP</label>
                    <input
                      type="number"
                      placeholder="mmHg"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Heart Rate</label>
                    <input
                      type="number"
                      placeholder="bpm"
                  ...
                  <div>
                    <label className="form-label block text-[10px] mb-1">Oedema</label>
                    <select
              ...
              {/* 2D Echo & ECG Section */}
              <div>
                <h4 className="text-[11px] font-bold uppercase tracking-wider text-amber-500 mb-2">2D Echocardiography & ECG Conduction</h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
                  <div>
                    <label className="form-label block text-[10px] mb-1">LVEF (%)</label>
                    <input
              ...
              {/* Labs Section */}
              <div>
                <h4 className="text-[11px] font-bold uppercase tracking-wider text-amber-500 mb-2">Blood Laboratory Values</h4>
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3">
                  <div>
                    <label className="form-label block text-[10px] mb-1">Creatinine</label>
                    <input
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-700/20">
                ...
                <button
                  type="submit"
                  disabled={savingEdit}
                  className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold shadow-lg shadow-amber-500/20 transition-all"
                >
                  <Save className="w-4 h-4" />
